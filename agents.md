...
The standard domains are the canonical list. Each standard hostname also has an exact MTO counterpart generated by prefixing the complete hostname with `mto.`:
...
Standard and MTO portals use the same `flight` syntax and shared pinned storage. The default "Go to Portal" destination remains `security.microsoft.com`.
